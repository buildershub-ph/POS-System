...
-- Receiving, transferring, and count-adjusting stock is now limited to the
-- owner and manager roles. Stock Employee (and every other role) can still
-- view the catalogue and current stock levels, but can no longer post any
-- transaction that changes on-hand quantities.
