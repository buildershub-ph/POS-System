...
-- ---------------------------------------------------------------------------
-- Lets an owner/manager record a sale under a date other than today -- for a
-- walk-in sale that was forgotten and only entered the next day. Re-creates
-- create_sale identical to migration 0021's version, plus an optional
-- saleDate that overrides created_at (and completed_at, for a completed
-- sale). Ignored for any role other than owner/manager, so a cashier can
-- never backdate a sale even if a request were crafted by hand.
-- ---------------------------------------------------------------------------
