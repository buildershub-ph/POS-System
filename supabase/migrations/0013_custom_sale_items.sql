...
-- Lets a cashier sell something that isn't in the catalogue at all -- a
-- customer order for an item the store neither stocks nor displays. It is
-- recorded on the sale for bookkeeping only; it never touches inventory,
-- since there was never any stock of it to begin with.
...
alter table public.sale_lines
  drop constraint if exists sale_line_variant_or_custom;
alter table public.sale_lines
  add constraint sale_line_variant_or_custom check (
    (variant_id is not null and custom_item_name is null)
    or (variant_id is null and custom_item_name is not null)
  );
...
begin
  ...
  for line in select value from jsonb_array_elements(p_sale->'lines') loop
    ...
    insert into public.sale_lines (
      sale_id, variant_id, custom_item_name, custom_sku, quantity, selling_unit,
      original_srp, actual_selling_price, discount_reason, approval_required,
      approved_by, approved_at
    ) values (
      sale_uuid, line_variant, line_custom_name, line_custom_sku, line_quantity, line_unit,
