...
-- A sale is allowed to take stock negative -- a display-only or currently
-- out-of-stock item can still be sold as a pre-order/backorder, to be
-- fulfilled by ordering more from the supplier. Every other stock-reducing
-- transaction (transfer, damaged, supplier_return) still requires enough
-- physical stock to actually exist, since those move real stock, not future
-- supply.
...
begin
  ...
  if transaction_type in ('receiving','transfer','supplier_return','damaged','display_stock','physical_count_adjustment')
     and actor_role not in ('owner','manager') then raise exception 'Role cannot post this inventory transaction'; end if;
  ...
  if transaction_type not in ('receiving','customer_return','physical_count_adjustment','reversal','sale') then
